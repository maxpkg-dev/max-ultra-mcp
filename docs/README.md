...
- The support footer keeps direct **maxpkg.dev** and **3dground.net** links and gives **Donate** a dedicated warm accent and icon instead of reusing a warning color. Footer links and Server labels copy an opaque `Parent.BackColor` only when it is consistent with the active Max theme; unrelated system-white host colors are rejected and ColorMan supplies the rollout fallback. Refresh uses the Lucide `refresh-cw` geometry, while Donate uses an antialiased vector heart; both are rasterized to disposable in-memory bitmaps, so no external image asset or SVG renderer is required. The Settings About section also identifies **maxpkg.dev** as the package manager with a direct link.
