...
- Only the bracketed category token, such as `[success]`, is highlighted: its background uses the category color and its text is white. The timestamp and message retain the category foreground color without a fill. Newline separators and the three trailing display-only lines keep the normal log background.
