...
- A compact **Donate** button sits between **AI setup** and **Hide panel** in the header. A separate left-aligned footer below the connection controls provides direct **maxpkg.dev** and **3dground.net** links; all panel links remain orange in their normal, hover, and visited states. The Settings About section also identifies **maxpkg.dev** as the package manager with a direct link.
...
- Final normal window `x`/`y` plus user-resized `width`/`height` and the explicit `hidden` state are saved before cleanup in the per-user scripts `MaxUltraMCP\panel-ui.ini` file. Existing position-only files remain compatible and default to expanded. On launch, corrupt or missing values fall back safely; size is constrained to the panel's 540×540 layout minimum and the selected monitor's current working area, and position is clamped so the full panel remains visible. If monitor topology or DPI scaling changed, the nearest current screen is used; an unusable position centers the validated size on the primary screen.
