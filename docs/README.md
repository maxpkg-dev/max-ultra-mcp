...
3. The **Activity** zone takes the remaining height. Its log fills the available width, with **Reconnect**, **Stop / Exit**, and **Settings** directly below it.
4. A 44 px support footer contains the project links and the visually distinct **Donate** action.
...
- Panel and log colors derive from 3ds Max `ColorMan` background/text colors, with safe fallbacks. The borderless RichTextBox is read-only, wrapped, capped at 30 entries, and preserves its colored success, warning, error, system, debug, and informational messages plus its existing scroll behavior. Native RichEdit margins provide six pixels of internal space on both horizontal sides. Three permanent empty display lines follow the latest entry, and new entries are inserted before them; protocol responses exclude this visual padding.
...
- The support footer keeps direct **maxpkg.dev** and **3dground.net** links and gives **Donate** a dedicated warm accent and icon instead of reusing a warning color. Footer links and Server labels copy an opaque `Parent.BackColor` only when it is consistent with the active Max theme; unrelated system-white host colors are rejected and ColorMan supplies the rollout fallback. Donate, Refresh, Hide panel, Reconnect, Stop / Exit, and Settings load packaged, ready-to-use white PNG renditions of their Lucide icons directly through WinForms. The icons are not rebuilt or recolored at runtime, and no SVG renderer is required. Hide panel uses the same 120 px width as Settings, and Reconnect uses the distinct `plug-zap` icon instead of duplicating Refresh. The Settings About section also identifies **maxpkg.dev** as the package manager with a direct link.
- Each category badge includes one filled space before `[` and after `]`. Status names are padded to seven characters and rendered in a dedicated monospaced font, giving all colored badges the same pixel width. An unfilled ` > ` separator and additional spacing align the following messages; these presentation characters never enter MCP activity responses. The badge background uses the category color and its text uses a darker, more saturated shade of the same hue; light themes receive a lighter badge surface for contrast. The timestamp and message retain the category foreground color without a fill. Newline separators and the three trailing display-only lines keep the normal log background.
- After the first successful bridge connection, the Activity log shows its first highlighted support reminder at the absolute ten-minute deadline. The next two reminders are scheduled at absolute 60-minute intervals from the preceding reminder, for a maximum of three per active bridge session. A delayed UI tick emits at most one due reminder, and Hide/Expand or Reconnect does not reset the schedule. The exact local-only message is: `Max Ultra MCP is maintained independently in personal time. Without community support, continued development may not be sustainable. If it helps your work, please use Donate to support it.`
