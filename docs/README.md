...
The MaxPkg release contains the general skill at `skills\max-ultra-mcp\SKILL.md`, the generic spline-modeling skill at `skills\max-ultra-spline-modeling\SKILL.md`, and the architectural plan skill at `skills\max-ultra-floor-plan\SKILL.md`. MCP registration does not silently copy skill files into an AI client profile. Clients that support skills can load or install any folder separately; clients without skill support continue to use the complete MCP schemas and initialization instructions.
