...
3. Two seconds after startup, the first-start script checks AI registration readiness once. When all supported client results are conclusive and no supported AI client is configured, it opens **AI Client Setup** automatically unless that automatic opening was dismissed; a failed check stays visible as **Check failed** without opening setup. On **1. Setup**, select **ChatGPT Desktop / Codex** and/or **Claude Code**, then choose **Install selected**. After reconnecting the client, use **2. Test prompt** to copy a safe, read-only connection test.
...
**Antigravity:** select its checkbox in AI Client Setup and click **Install selected**; setup backs up and updates its server settings automatically. See [Antigravity setup](docs/ANTIGRAVITY.md).
