...
async function runSmokeTest() {
  ...
  try {
    ...
    assert.match(bootstrapSource, /lblStatus .* pos: \[12,10\] width: 428/);
    assert.match(bootstrapSource, /lblContext .* pos: \[12,34\]/);
    assert.match(bootstrapSource, /button btnAgents "AI setup" pos: \[448,8\] width: 72 height: 22/);
    ...
    assert.match(bootstrapSource, /button btnSettings "Settings" pos: \[548,468\] width: 120 height: 30/);
    assert.match(bootstrapSource, /on btnSettings pressed do if \(bridgeClient != undefined\) do bridgeClient\.showSettingsDialog\(\)/);
    assert.match(bootstrapSource, /rtbActivity .* pos: \[4,62\] width: 672 height: 398/);
    assert.match(bootstrapSource, /button btnDonate "Donate" pos: \[528,8\] width: 60 height: 22/);
    assert.match(bootstrapSource, /shellLaunch "https:\/\/store\.payproglobal\.com\/checkout\?products\[1\]\[id\]=137366"/);
    assert.match(bootstrapSource, /hyperLink lnkMaxPkg "maxpkg\.dev" pos: \[12,509\][\s\S]*address: "https:\/\/maxpkg\.dev"/);
    assert.match(bootstrapSource, /hyperLink lnk3DGround "3dground\.net" pos: \[102,509\][\s\S]*address: "https:\/\/3dground\.net"/);
    assert.equal((bootstrapSource.match(/color: orange hoverColor: orange visitedColor: orange/g) || []).length, 4);
    ...
    assert.match(bootstrapSource, /btnDonate\.pos = \[panelWidth - 152, 8\]/);
    assert.match(bootstrapSource, /lnkMaxPkg\.pos = \[12, footerY \+ 1\]/);
    assert.match(bootstrapSource, /lnk3DGround\.pos = \[102, footerY \+ 1\]/);
    assert.match(bootstrapSource, /btnAgents\.pos = \[panelWidth - 232, 8\]/);
    assert.match(bootstrapSource, /btnHide\.pos = \[panelWidth - 84, 8\]/);
    assert.match(bootstrapSource, /lblStatus\.width = panelWidth - 252/);
    ...
    assert.match(bootstrapSource, /hyperLink lnkAboutPackageManager "maxpkg\.dev"[\s\S]*address: "https:\/\/maxpkg\.dev"/);
    ...
    assert.match(bootstrapSource, /rollout MaxUltraMcpStatusDialog "3DGROUND - Max Ultra MCP"/);
    assert.match(bootstrapSource, /UIAccessor\.SetWindowText statusDialog\.hwnd expectedTitle/);
    ...
    assert.match(bootstrapSource, /local minimumHeight = amin 540 maximumHeight/);
