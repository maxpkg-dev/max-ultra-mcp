...
async function runSmokeTest() {
  ...
  try {
    ...
    assert.doesNotMatch(bootstrapSource, /on MaxUltraMcp\w+ resized panelSize|handlePanelResized|ResizeEnd|MinimumSize/);
    ...
    assert.doesNotMatch(bootstrapSource, /resizePanelControls|panelLayoutInProgress|resolvedPanelSize|measuredChromeHeight|updateRolloutLayout panelFloater/);
    ...
    assert.match(bootstrapSource, /persistPanelPosition \(capturePanelPosition formSender\)/);
    ...
    assert.match(bootstrapSource, /fn normalizePanelPosition/);
    ...
    assert.match(bootstrapSource, /local candidateBounds = dotNetObject "System\.Drawing\.Rectangle" panelX panelY 720 640/);
    assert.match(bootstrapSource, /local maximumX = workingArea\.Right - 720[\s\S]*local maximumY = workingArea\.Bottom - 640/);
    ...
    assert.doesNotMatch(bootstrapSource, /(?:get|set)INISetting uiStateFilePath "panel" "(?:width|height)"/);
    ...
    assert.match(bootstrapSource, /if \(not isDisposed and rememberNormalPanelPosition formSender\)[\s\S]*persistPanelPosition lastNormalPanelPosition/);
    assert.match(bootstrapSource, /return normalizePanelPosition storedPosition/);
    ...
    assert.doesNotMatch(bootstrapSource, /local maximumY = workingArea\.Bottom - 64(?:\r?\n|$)/);
    ...
    assert.match(restorePanelBody, /restorePosition = if \(hiddenPanelPosition == undefined\) then loadPanelPosition\(\) else normalizePanelPosition hiddenPanelPosition/);
    assert.match(restorePanelBody, /if \(not panelIsOpen\(\)\) do \([\s\S]*this\.createPanelFloater restorePosition/);
    ...
    assert.match(hidePanelBody, /hiddenPanelPosition = capturePanelPosition panelForm/);
    assert.match(hidePanelBody, /persistPanelPosition hiddenPanelPosition/);
    ...
    assert.match(savedVisibilityBody, /hiddenPanelPosition = loadPanelPosition\(\)/);
    ...
    assert.match(formClosingBody, /persistPanelPosition \(capturePanelPosition formSender\)[\s\S]*persistPanelHiddenState false/);
