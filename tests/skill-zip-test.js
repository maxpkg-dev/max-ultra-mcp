...
try {
  ...
  const directZip = run(["add-zip", enclosed], store);
  assert.equal(directZip.succeeded[0].id, directZip.skills[0].id);
  assert.equal(directZip.skills[0].clients.length, 3);
  assert.match(run(["add-zip", enclosed], store).failed[0].reason, /SKILL_DUPLICATE/);
  store.remove(directZip.succeeded[0].id);
  const batchFirst = zip(valid, path.join(root, "first.zip"));
  const batchSecond = zip(valid.map((entry) => entry.name === "SKILL.md" ? { ...entry, text: skill.replace("zip-example", "zip-second") } : entry), path.join(root, "second.zip"));
  const corrupt = path.join(root, "corrupt.zip"); fs.writeFileSync(corrupt, "not a zip");
  const emptyRegistry = store.registry();
  assert.equal(run(["add-zips"], store).cancelled, true);
  assert.deepEqual(store.registry(), emptyRegistry);
  const batch = run(["add-zips", batchFirst, corrupt, batchFirst, batchSecond], store, { errorColor: "255,125,125", successColor: "120,225,150" });
  assert.deepEqual(batch.succeeded.map((item) => item.name), ["zip-example", "zip-second"]);
  assert.equal(batch.failed.length, 2);
  assert.match(batch.failed[1].reason, /SKILL_DUPLICATE/);
  assert.match(batch.html, /2 imported/);
  assert.equal((batch.html.match(/Restart your AI client/g) || []).length, 1);
  assert.match(batch.html, /color:rgb\(255,125,125\)/);
  assert.match(batch.html, /color:rgb\(120,225,150\)/);
  assert.match(batch.html, /border-left:3px solid #ff7f00/);
  assert.equal(batch.selectedId, undefined);
  assert.ok(batch.skills.every((item) => item.clients.length === 3));
  const allFailed = run(["add-zips", corrupt, batchFirst], store);
  assert.equal(allFailed.succeeded.length, 0);
  assert.equal(allFailed.failed.length, 2);
  assert.doesNotMatch(allFailed.html, /Restart your AI client/);
  for (const item of batch.succeeded) store.remove(item.id);
  const inputFile = path.join(root, "selection.inputs");
  fs.writeFileSync(inputFile, batchFirst + "\n");
  const singleBatch = run(["add-zips-file", inputFile], store);
  assert.equal(singleBatch.succeeded.length, 1);
  assert.equal(singleBatch.failed.length, 0);
  store.remove(singleBatch.succeeded[0].id);
