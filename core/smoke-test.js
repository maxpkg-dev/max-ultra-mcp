...
async function runSmokeTest() {
  ...
  try {
    ...
    assert.match(bootstrapSource, /GetCharIndexFromPosition firstClientPoint/);
    assert.match(bootstrapSource, /if \(savedSelectionLength > 0\) do statusDialog\.rtbActivity\.Select/);
    ...
    assert.ok(activityRefreshBody.indexOf("rtbActivity.ResumeLayout()") < activityRefreshBody.indexOf("if (shouldAutoScroll)"), "RichTextBox layout must resume before final scroll positioning");
    assert.ok(activityRefreshBody.indexOf("rtbActivity.Update()") < activityRefreshBody.indexOf("rtbActivity.ScrollToCaret()"), "RichTextBox must update before scrolling to the caret");
