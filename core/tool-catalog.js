...
const polygonMesh = {
  ...
  properties: {
    ...
    subdivisionReady: { type: "boolean", default: false },
    requireSingleShell: { type: "boolean", default: false },
  },
...
const tools = [
  ...
  tool("max_material_find_unassigned", "Find geometry with no material, invalid or missing material classes, empty Multi/Sub slots, or missing bitmap inputs without changing selection.", schema({ includeHidden: { type: "boolean", default: true }, includeFrozen: { type: "boolean", default: true }, includeXRefs: { type: "boolean", default: false }, includeNonGeometry: { type: "boolean", default: false }, includeMissingMaps: { type: "boolean", default: true }, limit: { type: "integer", minimum: 1, maximum: 200, default: 100 } }), readOnly, "archviz"),
  tool("max_add_normal_modifier", "Add one named Normal modifier to a guarded mesh node for an immediate screenshot A/B orientation review. Use max_undo after the comparison when the flipped variant should not be kept.", schema({ node: objectRef, flip: { type: "boolean", default: true }, dryRun: { type: "boolean", default: false } }, ["node"]), write, "archviz"),
  tool("max_add_smooth_modifier", "Add one named Smooth modifier to a guarded mesh node with Auto Smooth enabled and an explicit angular threshold.", schema({ node: objectRef, threshold: { type: "number", minimum: 0, maximum: 180, default: 30 }, preventIndirect: { type: "boolean", default: false }, dryRun: { type: "boolean", default: false } }, ["node"]), write, "archviz"),
  ...
  tool("max_capture_viewport", "Maximize and capture the active viewport as an MCP image using a temporary clean review preset. User display settings are restored; the viewport remains maximized.", schema({ width: { type: "integer", minimum: 1, maximum: 8192 }, height: { type: "integer", minimum: 1, maximum: 8192 }, reviewPreset: { type: "string", enum: ["clean-realistic", "clean-shaded", "clean-current"], default: "clean-realistic" } }), readOnly),
  ...
  tool("max_renderer_properties_get", "Inspect the active renderer through show renderers.current and return its available property names plus bounded diagnostic output.", schema({ maxChars: { type: "integer", minimum: 1024, maximum: 500000, default: 200000 } }), readOnly),
  ...
  tool("max_execute", "Execute unrestricted MaxScript. Prefer semantic tools when available and always provide a concise activity description for the 3ds Max log.", schema({ script: { type: "string", minLength: 1 }, activity: { type: "string", minLength: 3, maxLength: 80, description: "Concise imperative English action shown in the 3ds Max activity log, such as Create box, Delete objects, or Create layers. Do not include code or paths." }, timeout_ms: { type: "integer", minimum: 1000, maximum: MAX_EXECUTION_TIMEOUT_MS, default: 60000 } }, ["script"]), openWrite),
  tool("max_run_script", "Execute MaxScript text and return its value, type, character count, and bounded truncation status up to 500,000 characters. Always provide a concise activity description for the 3ds Max log.", schema({ script: { type: "string", minLength: 1 }, activity: { type: "string", minLength: 3, maxLength: 80, description: "Concise imperative English action shown in the 3ds Max activity log, such as Create box, Delete objects, or Create layers. Do not include code or paths." }, timeout_ms: { type: "integer", minimum: 1000, maximum: MAX_EXECUTION_TIMEOUT_MS, default: 60000 } }, ["script"]), openWrite),
  tool("max_run_script_file", "Execute a local .ms or .mse file inside 3ds Max. Always provide a concise activity description for the 3ds Max log.", schema({ filePath: { type: "string", minLength: 1 }, activity: { type: "string", minLength: 3, maxLength: 80, description: "Concise imperative English action shown in the 3ds Max activity log. Do not include code, filenames, or paths." }, timeout_ms: { type: "integer", minimum: 1000, maximum: MAX_EXECUTION_TIMEOUT_MS, default: 60000 } }, ["filePath"]), openWrite),
