...
### Simple rails and boards

Use the simplest construction that matches the real part. A straight rectangular rail or board, including a controlled taper on one side when supported by the reference, may start as a Box, converted to an editable cage with the necessary small bevels and support loops for one-iteration TurboSmooth. An equivalent simple polygon mesh is equally valid; no particular UI tool is required. This exception does not justify replacing a changing-section armrest or volumetric cushion with a generic box.

Preserve flat sides, straight edges, clean ends, and the intended constant section or controlled taper in the part's local geometry. A side rail between legs may taper while a plain front rail remains constant-section; follow the reference rather than imposing either on every rail. Set its overall angle and placement with a rigid object transform, not unrelated vertex dragging, warping, or projection onto another surface; define an intended taper coherently in local cross-sections before placement. Derive dimensions, bevel radius, and angle from the reference; no fixed example angle is a default. Verify the transformed part's section and end shape before and after smoothing. During final attachment, preserve the rigid placement and keep the part as an intentional Element in the one Editable Poly cage with live TurboSmooth.

Choose construction appropriate to each component; do not radially project a generic cubic grid onto a long thin rail or upright. Such projection can create diagonal edge bunching, stepped ends, or longitudinal rows pulled toward one face. Keep successive cross-sections ordered monotonically along the member and corresponding perimeter vertices in consistent order, without crossed or scrambled connections. After any end or section edit, recheck local face adjacency, winding, section correspondence, and spacing; update adjoining rows coherently rather than leaving long narrow strips or abrupt density jumps.
...
When neighboring rings describe neither a separate seam nor a distinct form change, test a local reduction while preserving a control version. Reducing three redundant rings to one is an example, not a fixed rule. Compare the original and candidate at one TurboSmooth iteration for silhouette, volume, highlights, and seam shape; keep the reduction only if those remain correct. Preserve real seam topology and restore the control if simplification loses necessary support.
